...
class Wykop {
  ...
  private createUrl(
    ...
    if (namedParams) {
      joinedNamedParams = Object.entries(namedParams)
    ...
    return `${baseUrl}${joinedApiParams}${joinedNamedParams}/appkey/${
      this.appKey
    }/userkey/${this.userKey}/data/${fullData ? 'full' : 'compact'}`;
  }

  private createApiSign(
    requestUrl: string,
    postParams?: { [key: string]: string | number }
  ): string {
    let postParamsString: string = '';
    if (postParams) {
      postParamsString = Object.keys(postParams)
        .map((key: string) => postParams[key])
        .join(',');
    }
